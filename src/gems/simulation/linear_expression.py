...
def is_close_abs(value: pd.DataFrame, other_value: float, eps: float) -> bool:
    return np.all(np.abs(value - other_value)) < eps


def is_zero(value: pd.DataFrame) -> bool:
    return is_close_abs(value, 0, EPS)


def is_one(value: pd.DataFrame) -> bool:
    return is_close_abs(value, 1, EPS)


def is_minus_one(value: pd.DataFrame) -> bool:
    return is_close_abs(value, -1, EPS)
...
class LinearExpression:
    ...
    def __init__(
        ...
        constant: Optional[pd.DataFrame] = None,
    ) -> None:
        self.terms = {}
        if terms:
        ...
        if constant is not None:
            ...
            # Need to create the constant DataFrame the same size as term coefficient
            for val in self.terms.values():
                shape = val.coefficient.shape
                break
            self.constant = pd.DataFrame(np.full(shape, 0))
        else:
            self.constant = pd.DataFrame([[0]])
