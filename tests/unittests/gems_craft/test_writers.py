...
from gems_craft.model.parsing import LibrarySchema, parse_yaml_library, write_yaml_library
from gems_craft.optim_config import load_optim_config, write_optim_config
...
from gems_craft.study.parsing import SystemSchema, parse_yaml_components, write_yaml_system
...
def test_write_yaml_system_roundtrip(tmp_path: Path) -> None:
    ...
    with original_path.open() as f:
        original = parse_yaml_components(f)

    out = tmp_path / "system.yml"
    write_yaml_system(original, out)

    with out.open() as f:
        reloaded = parse_yaml_components(f)
...
def test_write_optim_config_roundtrip(tmp_path: Path) -> None:
    ...
    write_optim_config(config, out)

    reloaded = load_optim_config(out)
...
def test_write_optim_config_creates_parent_dirs(tmp_path: Path) -> None:
    """write_optim_config creates missing parent directories."""
    config = OptimConfig()
...
def test_write_optim_config_uses_kebab_keys(tmp_path: Path) -> None:
...
# ---------------------------------------------------------------------------
# ScenarioBuilder.dump
# ---------------------------------------------------------------------------


def test_scenario_builder_dump_roundtrip(tmp_path: Path) -> None:
    ...
    reloaded = ScenarioBuilder.load(out)
...
def test_scenario_builder_dump_format(tmp_path: Path) -> None:
    """Dumped file uses 1-based column indices and correct line format."""
...
def test_scenario_builder_dump_creates_parent_dirs(tmp_path: Path) -> None:
    """ScenarioBuilder.dump creates missing parent directories."""
    sb = ScenarioBuilder()
...
def test_scenario_builder_dump_load_existing_fixture(tmp_path: Path) -> None:
    """Load an existing fixture, dump it, reload it, verify identity."""
    fixture = (
    ...
    original = ScenarioBuilder.load(fixture)
    out = tmp_path / "modeler-scenariobuilder.dat"
    original.dump(out)
    reloaded = ScenarioBuilder.load(out)
