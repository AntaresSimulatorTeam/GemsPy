...
- `SimulationTableWriter` (`simulation_table_writer.py`): writes a `SimulationTable` as CSV or Parquet, one file per scenario, handed over per scenario via `SimulationSession(on_scenario_done=...)`: as each scenario is solved in sequential/parallel modes, and in parallel threads after the single solve in frontal mode, built on demand by `SimulationTableBuilder.build_per_scenario`
